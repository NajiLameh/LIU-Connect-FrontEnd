...
import { InsertInternshipToDataBase } from '../../BackEnd/InternshipQueries/InsertInternshipToDataBase';
...
export default function AddNewInternship({ navigation }) {
  ...
  const [type, setType] = useState('Select Internship Type');
  const [location, setLocation] = useState('');
  const [paid, setPaid] = useState('Select Payment Option');
  ...
  const addInternshipHandler = async () => {
    ...
    if (invalidEntries) {
      ...
      try {
        ...
        userId = await userID();
        const response = await InsertInternshipToDataBase(
          userId,
          name,
          formattedDate,
          company,
    ...
    <>
      ...
      <KeyboardAvoidingViewWrapper>
        <View style={styles.screenContainer}>
          ...
          />
          <InputField
            placeHol="Duration (Optional)"
            value={duration}
            setValue={setDuration}
            marginV={10}
          />
          <InputField
            placeHol="Internship Link (Optional)"
